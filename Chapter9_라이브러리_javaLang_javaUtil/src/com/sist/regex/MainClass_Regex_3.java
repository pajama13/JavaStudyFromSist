/*


    <div class="thumb_cont">
       <strong class="tit_item">
         <a href="/moviedb/main?movieId=149380" class="link_txt" data-tiara-layer="moviename">올빼미</a>
       </strong>

 */
package com.sist.regex;
import java.util.*;
import java.util.regex.*;

import org.jsoup.Jsoup;
import org.jsoup.nodes.Document;
public class MainClass_Regex_3 {

	public static void main(String[] args) {
		String[] movie={
				"올빼미",
				"블랙 팬서: 와칸다 포에버",
				"동감",
				"유포자들",
				"데시벨",
				"스트레인지 월드",
				"원피스 필름 레드",
				"창밖은 겨울",
				"폴: 600미터",
				"그녀가 말했다"				
		};
		Scanner scan=new Scanner(System.in);
		System.out.println("-------- 영화목록 --------");
		for(int i=0; i<movie.length;i++)
		{
			System.out.println((i+1)+". "+movie[i]);
		}
		System.out.println("------------------------");
		System.out.print("영화 선택 : ");
		int movieId=scan.nextInt();
		
		String title=movie[movieId-1]; // 배열 0번이 첫번째여서 -1 해주기
		System.out.println("선택된 영화 : "+title);
		
		//동영상
		try
		{
			String url="https://www.youtube.com/results?search_query="+title; //올빼미 유튜브 
			Document doc=Jsoup.connect(url).get(); //네트워크 연결 - 예외처리 필수
			String pattern="/watch\\?v=[^가-힣]+"; //자바에서 사용하는 기호 (!,?,.등)를 문자로 쓸 때는 역슬래쉬 앞에 쓰기
			String data=doc.toString(); //data안에서 pattern찾기
			Pattern p=Pattern.compile(pattern);
			Matcher m=p.matcher(data);
			String key="";
			int i=0;
			while(m.find())
			{
				if(i==1)
				{
				key=m.group();
				break;
				}
				i++;
			}
			//키만 잘라내기
			key=key.substring(key.indexOf("=")+1,key.indexOf("\""));
			//System.out.println(key);
			Runtime.getRuntime().exec("C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe https://www.youtube.com/embed/"+key);
		}catch(Exception ex){}
		
		//JD-TFT6FOY0
		
	//
	}
	
//
}
